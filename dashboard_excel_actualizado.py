...
if ventas_file and productos_file:
    ...
    with col1:
        fig1 = px.bar(top_productos, x='Volumen', y='Producto', orientation='h', title="Top 10 productos por volumen")
        st.plotly_chart(fig1, use_container_width=True)
    with col2:
        fig2 = px.pie(top_lineas, names='LOB', values='Volumen', title="Distribución por línea de negocio", hole=0.4)
    ...
    with col4:
        fig4 = px.bar(vol_por_um, x='UM', y='Volumen total', title="Volumen vendido por UM")
    ...
    if inventarios_file:
        ...
        merged = resultado.merge(inventario_df[['Material', 'Inventario Lts.']].rename(columns={'Material': 'SKU'}), on='SKU', how='left')
        merged['Inventario Lts.'] = merged['Inventario Lts.'].fillna(0)
        merged['Recomendaciones'] = merged['pronostico'] + merged['Punto_Reorden'] - merged['Inventario Lts.']
...
LUBCEN = load_all_data(ventas_file, productos_file)
st.write("Columnas disponibles en LUBCEN:", LUBCEN.columns.tolist())